...
function findWordCombinations(digits) {
  ...
  for (let i = 0; i <= digits.length; i += 1) {
    const firstHalf = findLetterCombinations(digits.slice(0, i));
    const secondHalf = findLetterCombinations(digits.slice(i));
    for (let j = 0; j <= firstHalf.length; j += 1) {
      for (let k = 0; k <= secondHalf.length; k += 1) {
        const currentWord = firstHalf[j];
        const currentMatch = secondHalf[k];
        ...
        if (currentWord && currentWord.word.length < digits.length) {
          numbersAndWords[currentWord.word] =
            currentWord.word + digits.slice(i);
        }
        if (currentMatch && currentMatch.word.length < digits.length) {
          numbersAndWords[currentMatch.word] =
            digits.slice(0, i) + currentMatch.word;
        }
      }
    }
  }
  ...
  const arrayOfKeys = Object.keys(numbersAndWords).sort(

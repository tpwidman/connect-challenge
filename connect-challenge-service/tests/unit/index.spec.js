...
test.serial("Testing app handler - db throws error", async (t) => {
  awsMock.mock("DynamoDB", "putItem", async () => {
    throw new Error("Something went wrong");
  });
  const err = await t.throwsAsync(() =>
    app(event, { db: new AWS.DynamoDB(), tableName: "ConnectChallenge" })
  );
  t.true(err instanceof Error);
});
test.serial("Testing app handler with a funky phone number", async (t) => {
  const eventWithFunkyPhoneNumber = _.cloneDeep(event);
  eventWithFunkyPhoneNumber.Details.ContactData.CustomerEndpoint.Address =
    "1111111";
  awsMock.mock("DynamoDB", "putItem");
  const result = await app(eventWithFunkyPhoneNumber, {
    db: new AWS.DynamoDB(),
    tableName: "ConnectChallenge",
  });
...
test.serial("Testing app handler - error handling", async (t) => {
  const err = await t.throwsAsync(() =>
    app(event, { db: {}, tableName: "ConnectChallenge" })
  );
  t.true(err instanceof Error);
});
